...
const hoverStyle = computed(() => {
  return {
    color: "#C9002B", // Hover color
  };
});
const clickStyle = computed(() => {
  return {
    color: "white", // Click color
  };
});
    ...
    >
      ...
      <div v-if="!node.heading && !node.sub" @click="linkClicked($event)">
        <RouterLink
          ...
          :style="node.style"
          :class="{ 'hover-effect': !props.horizontalHover }"
        >
          <i
            v-if="node.icon"
          ...
          ></i>
          <!-- Add this line to apply icon styles -->
          <span
            v-if="node.name"
            ...
            >{{ node.name }}</span
          >
          <!-- Add this line to apply text styles -->
          <span
            v-if="node.badge"
